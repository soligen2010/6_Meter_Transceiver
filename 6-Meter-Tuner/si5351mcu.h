/*
 * Modified by Dennis Cabell WC8C Janyary 2022:
 * 
 *   Created inheriting class Si5351mcu2 that uses software I2C
 * 
...
class Si5351mcu {
    ...
    private:
        ...
        virtual void i2cWrite(uint8_t, uint8_t);
...
class Si5351mcu2 : public Si5351mcu
{ 
  public:
    init(uint32_t nxtal, uint8_t sda, uint8_t scl) {
      // set the new base xtal freq
      base_xtal = int_xtal = nxtal;
  
      // start I2C (wire) procedures
      SWire = SoftwareWire(sda, scl, false, false);
      SWire.begin();
  ...
  private:
     ...
     ***************************************************************************/
    void i2cWrite(uint8_t regist, uint8_t value) {
        SWire.beginTransmission(SIADDR);
        SWire.write(regist);
        SWire.write(value);
